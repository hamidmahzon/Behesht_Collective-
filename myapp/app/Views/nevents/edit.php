...
?>
    ...
    <form class="w3-form w3-modal-content w3-animate-opacity" action="<?=base_url('edit/nevents/'.$record[0]->id)?>" enctype="multipart/form-data" method="post" style="background: linear-gradient(<?=$gcolor?>)">
        <?= csrf_field() ?>
        <div class="w3-panel w3-xlarge w3-center <?=$ncolor?>"><?=lang('app.editPost')?>: <?=$record[0]->$name?></div>
    <div class="w3-container w3-row-padding">
        <div class="w3-third w3-section w3-<?=$align?>">
            <label class="w3-label w3-large" for="namep"><?=lang('app.namep')?></label>
            <input type="text" name="namep" id="namep" required class="w3-input w3-border" value="<?=$record[0]->namep?>" placeholder="<?=lang('app.namep')?>" autofocus />
        </div>
        <div class="w3-third w3-section w3-<?=$align?>">
            <label class="w3-label w3-large" for="namee"><?=lang('app.namee')?></label>
            <input type="text" name="namee" id="namee" required class="w3-input  w3-border" value="<?=$record[0]->namee?>" placeholder="<?=lang('app.namee')?>" />
        </div>
        ...
        <div class="w3-half w3-section w3-<?=$align?>">
            ...
            <textarea class="w3-input" name="descp" id="contentp" rows="8" required><?=$record[0]->descp?></textarea>
        </div>
        <div class="w3-half w3-section w3-<?=$align?>">
            <label class="w3-label w3-large" for="desce"><?=lang('app.desce')?></label>
            <textarea class="w3-input" name="desce" id="contente" rows="8" required><?=$record[0]->desce?></textarea>
        </div>
